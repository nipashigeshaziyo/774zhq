	...
	<div class="box">
		...
		<div class="content">
			...
			<div class="show">
				...
				%>
				<form class="block" action="<%=request.getContextPath()%>/Update" method="post">
					书名：<input class="ip" type="text" name="name" value="<%=book.getName() %>"/><span><%if(error!=null)%><%=error.get(0)%></span><br/>
					...
					作者：<input class="ip" type="text" name="author" value="<%=book.getAuthor() %>"/><span><%if(error!=null)%><%=error.get(2)%></span><br/>
					...
					价格：<input class="ip" type="text" name="price" value="<%=book.getPrice() %>"><span><%if(error!=null)%><%=error.get(3)%></span><br/>
					数量：<input class="ip" type="text" name="count" value="<%=book.getCount() %>"><span><%if(error!=null)%><%=error.get(4)%></span><br/>
					简介：<textarea cols="30" rows="3" name="introduce"><%=book.getIntroduce() %></textarea><span><%if(error!=null)%><%=error.get(5)%></span><br/>
