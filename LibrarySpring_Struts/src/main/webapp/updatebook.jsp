...
<body>
	<div class="wrap">
		...
		<div class="content">
			...
			<div class="show">
				<form class="block" action="updateBookAction?bid=${book.bid }" method="post">
					书名：<input class="ip" type="text" name="name" value="${book.name}"/>
						<font color="red">
							<s:fielderror>
					...
					<c:forEach var="arr" items="${types}">
						...
						<br/>
					作者：<input class="ip" type="text" name="author" value="${book.author }"/>
						<font color="red">
							<s:fielderror>
						...
						<c:forEach items="${prices}" var="p">
							<input type="radio" name="discount" value="${p }"
								<c:if test="${book.discount eq p }">checked="checked"</c:if>
							/>${p}
						</c:forEach>
						...
						<br/>
						价格：<input class="ip" type="text" name="price" value="${book.price }">
						<font color="red">
							<s:fielderror>
						...
						<br/>
						数量：<input class="ip" type="text" name="count" value="${book.count }">
						<font color="red">
							<s:fielderror>
						...
						<br/>
						简介：<textarea cols="30" rows="3" name="introduce"">${book.introduce }</textarea>
						<font color="red">
							<s:fielderror>
